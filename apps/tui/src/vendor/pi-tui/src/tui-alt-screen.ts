...
export class TuiAltScreen extends TuiBase implements ViewportTUI {
	...
	/** MUON EDIT — see `setChildInputGuard`. */
	private childInputGuard: ((data: string) => boolean) | undefined;
	...
	/**
	 * MUON EDIT — see `handleViewportInput`.
	 *
	 * Set a predicate that answers "would a live child receive these bytes?".
	 * While it says yes, the alt screen's own viewport handling stands down.
	 * Unset (the default) the engine behaves exactly as it always did.
	 */
	setChildInputGuard(guard: ((data: string) => boolean) | undefined): void {
		this.childInputGuard = guard;
	}
	...
	private handleViewportInput(data: string): { consume?: boolean } | undefined {
		...
		// MUON EDIT — the viewport must not outrank a live child.
		//
		// Everything below this line consumes input for the alt screen's OWN
		// scrollback: mouse reports, and the `pageUp`/`pageDown`/`home`/`end`/
		// `ctrl+shift+up`/`ctrl+shift+down` bindings. Input listeners run
		// BEFORE the focused component, so a `{consume:true}` here ends the
		// dispatch and the component never sees the key.
		//
		// That is correct for an app whose document IS the scroll view. MUON's
		// desk renders exactly one screen with no scrollback, and hosts real
		// terminals inside it — so those keys were being taken from a vendor
		// CLI by a scroll view that had nothing to scroll. Home and End, dead
		// in someone else's editor, with no error and nothing on screen.
		//
		// The host supplies a predicate over the ACTUAL bytes. When it says a
		// child would receive them, this listener declines and lets them
		// through. Focus events above stay consumed either way: the engine
		// needs them to drop its selection state, and they are not keys.
		if (this.childInputGuard?.(data)) return undefined;
