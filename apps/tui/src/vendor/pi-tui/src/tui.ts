...
export abstract class TuiBase extends Container implements TUI {
	...
	/** MUON EDIT — see `setInputGate`. */
	private inputGate: ((data: string, emit: (out: string) => void) => void) | undefined;
	...
	start(): void {
		...
		this.terminal.start(
			(data) => {
				// MUON EDIT — see `setInputGate`. The gate may hold a partial
				// escape sequence and deliver it later, so it emits rather than
				// returns.
				if (this.inputGate) this.inputGate(data, (out) => this.handleTerminalInput(out));
				else this.handleTerminalInput(data);
			},
			() => this.requestRender(),
		);
	 ...
	 * MUON EDIT.
	 *
	 * Interpose on the RAW terminal read, before any listener or component
	 * sees it. A terminal splits its reads wherever the pipe happened to be
	 * full, so a chord can arrive in two pieces; without somewhere to hold the
	 * first piece, the engine dispatches half a sequence and then types the
	 * other half into the child. The gate emits rather than returns because
	 * releasing held bytes is asynchronous — a lone ESC is only known to be
	 * the Escape key once nothing follows it.
	 *
	 * Unset (the default) the engine behaves exactly as it always did.
	 */
	setInputGate(gate: ((data: string, emit: (out: string) => void) => void) | undefined): void {
		this.inputGate = gate;
	}
